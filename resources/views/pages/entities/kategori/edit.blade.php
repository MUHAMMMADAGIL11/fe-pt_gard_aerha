...
@section('content')
    <div class="space-y-8 px-4 sm:px-6 lg:px-8">
        <div>
            ...
            <h1 class="text-2xl sm:text-[26px] font-bold text-white mt-3">Edit Kategori</h1>
            <p class="text-sm text-slate-300 mt-1.5">Perbarui informasi kategori.</p>
        </div>

        <div class="bg-[#0F2536] rounded-2xl shadow-[0_18px_45px_rgba(0,0,0,0.45)] border border-white/5 p-6 sm:p-8">
            <form method="POST" action="{{ route('kategori.update', $kategori->id_kategori) }}" class="space-y-6" data-loading="true">
                @csrf
                @method('PUT')

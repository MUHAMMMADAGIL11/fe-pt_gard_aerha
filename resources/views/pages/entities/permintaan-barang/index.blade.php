...
@section('content')
    <div class="space-y-8 px-2 sm:px-4">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-[26px] font-bold text-white mt-3">Permintaan Barang</h1>
                <p class="text-[14px] text-slate-300 mt-1.5">
                    ...
                    <tbody class="divide-y divide-white/5 text-slate-100">
                        @forelse ($permintaan as $item)
                            <tr class="hover:bg-white/5 transition">
                                <td class="px-7 py-3.5 text-slate-300">{{ \Carbon\Carbon::parse($item->created_at ?? now())->format('d/m/Y') }}</td>
                                ...
                                <td class="px-7 py-3.5 text-center font-semibold text-white">{{ number_format($item->jumlah_diminta) }} pcs</td>
                                    ...
                                    <td class="px-7 py-3.5 text-right space-x-2">
                                        @if($item->status === 'Menunggu Persetujuan')
                                            <form method="POST" action="{{ route('permintaan-barang.approve', $item->id_permintaan) }}" class="inline" data-loading="true">
                                                @csrf
                                                <button type="submit"
                                                    class="inline-flex items-center rounded-md border border-emerald-200/40 px-3 py-1 text-[12px] font-semibold text-emerald-200 hover:bg-emerald-200/10">
                                                    Setujui
                                                </button>
                                            </form>

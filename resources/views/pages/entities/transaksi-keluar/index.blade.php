...
@section('content')
    <div class="space-y-8 px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-2xl sm:text-[26px] font-bold text-white mt-3">Barang Keluar</h1>
                <p class="text-sm text-slate-300 mt-1.5">Catat semua transaksi barang keluar dari gudang.</p>
            </div>
                ...
                <table class="min-w-full divide-y divide-white/5 text-[14px] text-slate-100 whitespace-nowrap">
                    <thead class="bg-[#152c3f] text-left text-slate-300 text-[12px] uppercase tracking-wide">
                        <tr>
                            ...
                            <th class="px-4 py-3 md:px-7 md:py-4 font-semibold hidden md:table-cell">Tujuan</th>
                    ...
                    <tbody class="divide-y divide-white/5 text-slate-100">
                        @forelse ($transaksi as $item)
                            <tr class="hover:bg-white/5 transition">
                                ...
                                <td class="px-4 py-3 md:px-7 md:py-3.5 text-slate-300 hidden md:table-cell">{{ $item->transaksiKeluar->tujuan ?? '-' }}</td>
